(() => {
  ...
  const goal = (name) => { const analytics = window.alexuysAnalytics; if (analytics && typeof analytics.goal === 'function') analytics.goal(name); };
  ...
  $$('[value="cancel"]').forEach((el) => el.addEventListener('click', (event) => { event.preventDefault(); el.closest('dialog')?.close(); }));
