...
# The old stage145 gallery runtime injects a canvas whose positioning CSS was
# scoped to the retired x135 family. Without that family attribute the canvas
# becomes a normal document-flow element and can grow the page on every resize.
html = re.sub(
...
/* /cases/ must remain a normal document-flow page: no legacy WebGL spacer. */
...
main.p130-hub[data-stage130-hub="cases"] .p130-hero{
  ...
  padding:clamp(42px,4vw,64px) 0 clamp(52px,5vw,76px)!important;
...
main.p130-hub[data-stage130-hub="cases"] .p130-list-section{
  min-height:0!important;
  height:auto!important;
  padding:0 0 clamp(80px,7vw,112px)!important;
  transform:none!important;
  translate:none!important;
}
...
if "stage145-cases-gallery-interactive.js" in final:
    raise SystemExit("stage166: legacy stage145 JS still present")
if "stage145-cases-gallery-interactive.css" in final:
    raise SystemExit("stage166: legacy stage145 CSS still present")
