...
STYLE = '''<style data-stage75-css="true">
...
for route in TARGETS:
    ...
    if 'data-stage75-css="true"' not in text:
        text, n = re.subn(r'</head>', STYLE + '</head>', text, count=1, flags=re.I)
        if n != 1:
            raise SystemExit(f"stage75: head close not found exactly once: {route}")
...
# Honest lastmod only for pages whose visible content changed.
